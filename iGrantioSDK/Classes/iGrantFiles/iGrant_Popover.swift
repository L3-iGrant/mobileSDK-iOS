...
public enum iGrant_PopoverOption {
...
open class iGrant_Popover: UIView {
  ...
  public init(options: [iGrant_PopoverOption]?, showHandler: (() -> ())? = nil, dismissHandler: (() -> ())? = nil) {
    super.init(frame: .zero)
    self.backgroundColor = .clear
  ...
  open func show(_ contentView: UIView, point: CGPoint, inView: UIView) {
    if self.dismissOnBlackOverlayTap || self.showBlackOverlay {
      ...
      if self.dismissOnBlackOverlayTap {
        self.blackOverlay.addTarget(self, action: #selector(iGrant_Popover.dismiss), for: .touchUpInside)
      }
    }
...
private extension iGrant_Popover {

  func setOptions(_ options: [iGrant_PopoverOption]?){
